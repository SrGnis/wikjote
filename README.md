# Wikjote
### A python package to parse the Spanish Wiktionary into JSON
...
The parsing has two steps:
1. Wikjote reads de zim file and parses each html page into a simplified json representation.
...
The idea is that the parsing and postprocessing can be configured by config files and light python coding without needing to rebuild the package. This is not implemented yet but the parsing configuration **is being worked on**.
...
The JSON resulting from the parsing will have the following schema:
```json
{
    "lema1": [
        {
