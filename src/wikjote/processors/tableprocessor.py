                    ...
                    if True:
                        ...
                        if element.tag == "th":
                            ...
                            if col_index <= len(top_headers) - 1:
                                if col_index == 0:
                                    top_headers[col_index] = self.object.get_all_text(
                                        element
                                    )
                                else:
                                    top_headers[
                                        col_index
                                    ] += "," + self.object.get_all_text(element)
                            else:
                                top_headers.insert(
                                    col_index, self.object.get_all_text(element)
                                )
                        if element.tag == "td":
                            if row_has_th:
                                header = ",".join(
                                    [top_headers[0], top_headers[col_index]]
                                )
                            else:
